...
:: option-1, run task def directly
:: option-2, run task def from a service
...
aws ecs create-service --cluster %CLUSTERNAME% --service-name %SERVICENAME% --task-definition %TASKDEFNAME% --desired-count %DESIREDTASKCOUNT% --launch-type "FARGATE" --network-configuration "awsvpcConfiguration={subnets=[%SUBNET%],securityGroups=[%SECURITYGROUP%],assignPublicIp=ENABLED}"
